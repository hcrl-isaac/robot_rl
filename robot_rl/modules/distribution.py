...
class TangentStepVonMisesFisherDistribution(VonMisesFisherDistribution):
    r"""A vMF whose mean is a geodesic step from a base direction, for steering policies on the sphere.

    The MLP output is a tangent step :math:`s` at the base direction :math:`u` (set per call through :attr:`base`,
    e.g. the policy's current latent): with :math:`v = s - (s \cdot u) u` and :math:`\theta = \min(|v|, \theta_{max})`,
    the mean direction is :math:`\cos\theta\, u + \sin\theta\, v/|v|`. A zero step keeps the base direction, so the
    policy starts as "hold" and the noise stays angular (the vMF concentration), never radial. Without a usable base
    (a fresh episode's zero latent) the mean is the normalized step itself.
    """

    def __init__(self, output_dim: int, step_scale: float = 1.0, max_step: float = 1.0, **kwargs: Any) -> None:
        """Initialize the distribution.

        Args:
            output_dim: Ambient dimension ``p`` (the sphere is ``S^{p-1}``). Must be even.
            step_scale: Multiplier on the MLP's tangent step before it is read as an arc length in radians.
            max_step: Clamp on the arc length of one step (radians).
            **kwargs: Forwarded to :class:`VonMisesFisherDistribution`.
        """
        super().__init__(output_dim, **kwargs)
        self.step_scale = step_scale
        self.max_step = max_step
        self.base: torch.Tensor | None = None

    def mean_direction(self, mlp_output: torch.Tensor) -> torch.Tensor:
        """Map the tangent step to the unit mean direction, given the current :attr:`base`."""
        if self.base is None:
            return torch.nn.functional.normalize(mlp_output, dim=-1)
        base = self.base
        if base.shape != mlp_output.shape:
            base = base.expand_as(mlp_output)
        unit = torch.nn.functional.normalize(base, dim=-1)
        step = self.step_scale * mlp_output
        tangent = step - (step * unit).sum(dim=-1, keepdim=True) * unit
        length = tangent.norm(dim=-1, keepdim=True)
        angle = length.clamp(max=self.max_step)
        stepped = torch.cos(angle) * unit + torch.sin(angle) * tangent / length.clamp(min=1e-8)
        fresh = base.norm(dim=-1, keepdim=True) < 1e-6
        return torch.where(fresh, torch.nn.functional.normalize(mlp_output, dim=-1), stepped)

    def update(self, mlp_output: torch.Tensor) -> None:
        """Update the distribution: mean = geodesic step from the base, concentration = kappa param."""
        self._mu = self.mean_direction(mlp_output)
        self._kappa = torch.exp(self.log_kappa).clamp(self.kappa_range[0], self.kappa_range[1])
        self._log_norm, self._a_ratio = self._bessel_terms(self._kappa)

    def deterministic_output(self, mlp_output: torch.Tensor) -> torch.Tensor:
        """Return the mean direction (unit vector) for the current base."""
        return self.mean_direction(mlp_output)

    def as_deterministic_output_module(self) -> nn.Module:
        """Not exportable on its own: the mean depends on the base direction supplied at call time."""
        raise NotImplementedError("TangentStepVonMisesFisherDistribution needs the base latent; export the model.")
